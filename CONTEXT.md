...
| mode | `link` or `copy`: how a skill reaches a skills directory. | form, verb, kind |
| managed | Link or copy created by `ski`. | owned, tracked |
| unmanaged | Path in a skills directory that `ski` did not create. `ski` refuses to replace or delete it. | foreign, stray, existing |
...
| missing | Recorded, but no link or copy is present in any skills directory. | not linked, not installed, absent |
...
| destination | Where a skill is written: scope, agents, and mode (link or copy). | target, placement |
| location | Where an installed skill already is: its mode and agents. | placement, presence |
