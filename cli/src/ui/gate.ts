...
export const reviewSkills = async <T extends Reviewable>(
  ...
  for (const entry of selection) {
    ...
    const outcome = await reviewSkill(name, files, yes);
    if (outcome === "pass") {
      approved.push(entry);
    } else {
...
export const stopsOn = (findings: Finding[], yes: boolean | undefined): Severity | null => {
...
const reviewSkill = async (
  ...
  const stop = stopsOn(findings, yes);
  if (!stop) return "pass";
